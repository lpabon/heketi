...
func (v *VolumeEntry) ReplaceBrick(
	db *bolt.db,
	allocator Allocator,
	executor executors.Executor,
	...
	if !utils.SortedStringHas(originalBrickIds, failedBrickId) {
		return logger.LogError("%v not found in volume %v",
			failedBrickId, v.Info.Id)
	}
	brickIds := utils.SortedStringsDelete(originalBrickIds, failedBrickId)
	...
	// Create brick
	err = CreateBricks(db, executor, []*BrickList{replacementBrickEntry})
	if err != nil {
		return err
	}

	// Send replacement brick to glusterfs
	logger.Debug("Send replacement command to gluster")

	// Remove failed device from db
	err = db.Update(func(tx *bolt.Tx) error {
		replacementBrickEntry.Save(tx)

		// Remove older brick from db
		err = v.removeBrickFromDb(tx, failedBrickEntry)
		if err != nil {
			return err
		}
